...
describe('boot token parity with @civitai/theme', () => {
  it('the DARK literals match the package [data-theme=dark] block', () => {
    ...
    expect(bootValue(':root', '--gm-boot-body')).toBe(body);
    expect(bootValue(':root', '--gm-boot-text')).toBe(text);
    expect(bootValue(':root', '--gm-boot-surface')).toBe(surface);
  });
  ...
  it('the LIGHT literals match the package :root block', () => {
    ...
    expect(tokenValue(media, ':root', '--gm-boot-body')).toBe(body);
    expect(tokenValue(media, ':root', '--gm-boot-text')).toBe(text);
    expect(tokenValue(media, ':root', '--gm-boot-surface')).toBe(surface);
  });
  ...
  it('no light value is reachable without an explicit light signal', () => {
    ...
    expect(bootValue(':root', '--gm-boot-body')).toBe(darkBody);
    expect(bootValue(':root', '--gm-boot-body')).not.toBe(lightBody);
