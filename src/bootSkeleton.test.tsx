...
/**
 * `<App>` mounted the way src/main.tsx mounts it, minus the host.
 *
 * 🔴 THE PROVIDER IS NOT TEST SCAFFOLDING — `App` calls `useToast()`, which throws
 * outside a `<ToastProvider>`, so this mirrors the real tree rather than inventing
 * one. Deliberately NO `<Harness>`: the harness posts BLOCK_INIT and would make
 * `ready` true, which is the opposite of the state under test here.
 */
function renderPreReady() {
  return render(
    <ToastProvider>
      <App />
    </ToastProvider>,
...
// `renderPreReady()` cannot mount at all. Stubbing it lets the transport construct
...
describe('the pre-ready paint agrees with the skeleton', () => {
  ...
  it('paints DARK before ready, not the SDK sentinel', () => {
    const { container } = renderPreReady();
